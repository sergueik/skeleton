...
public class Skeleton extends Builder {
    ...
    public Skeleton(JobSource jobSource, boolean downloadFiles) {
    ...
    private boolean verifyFile(File jobFile, AbstractBuild<?, ?> build, ConsoleLogger console) {
        ...
        try {
            if (!fp.exists()) {
                log(console, "[Skeleton] ERROR: Job file " + fp.getName() + " doesn't exists on channel" + fp.getChannel() + "!");
                //build.setResult(Result.FAILURE);
                return false;
            }
        } catch (IOException e) {
            log(console, "[Skeleton] ERROR: failed to verify that " + fp.getName() + " exists on channel" + fp.getChannel()
        ...
        try {
            ...
            log(console, "[skeleton] ERROR: Could not get canonical path to workspace:" + ioe);
            ioe.printStackTrace();
            build.setResult(Result.FAILURE);
        }
        return jobFile;
    ...
    public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener)
        ...
        log(console, "[Skeleton] INFO: Job file  " + jobFile.getName()+ " prepared");
        ...
        if (jobSource == null) {
            ...
            return false;
								...
								try {
            String[] cmd = buildCommandLine(build,listener,new FilePath(build.getWorkspace(), jobFile.getPath()),false);
            int result;
            try {
                ...
                for(String c:cmd){
                    ...
                    sb.append(" ");
								...
								result = 0;
                result = launcher.launch().cmds(cmd).envs(envVars).stdout(listener).pwd(build.getWorkspace()).join();
            } catch (IOException e) {
               // Util.displayIOException(e,listener);
    ...
    public DescriptorImpl getDescriptor() {
        ...
        String cmd = "powershell.exe";//last hope in case of missing or not selected installation
        /* 
        hudson.plugins.groovy.GroovyInstallation installation = getGroovy();
        if(installation != null) {
